    ...
    <div>
        ...
        >
            ...
            <el-table-column
                ...
                <el-table-column
                        prop="outputBaozhuang"
                        label="包装" show-overflow-tooltip>
                </el-table-column>
                <el-table-column
                        prop="outputBaozhuangTotalPrice"
                        label="包装总价格" show-overflow-tooltip>
                </el-table-column>
                <el-table-column
                prop="outputTeding"
                ...
                <el-table-column
                    prop="outputTedingTotalPrice"
                    label="特定总价格" show-overflow-tooltip>
                </el-table-column>
                <el-table-column
                        prop="outputBeijing"
    ...
    export default {
        ...
        methods: {
            ...
            async querySearchAsync(queryString,cb) {
                let data = await this.$apis.output_get(this.currentPage, this.pageSize, queryString,this.month,this.year)
            ...
            async exportMonth(){
                if(this.inOutValue){
                    ...
                    let data = await this.$apis.output_file(date.getMonth()+1, date.getFullYear())
                    console.log("导出", data)
                    const blob = new Blob([data])
                    const fileName = `${date.getFullYear()}年${date.getMonth()+1}月产值数据`
            ...
            getSummaries(param){
                ...
                columns.forEach((column, index, array) => {
                    if(index===1){
                        ...
                        if(column.property === "outputBaozhuangTotalPrice" || column.property === "outputTedingTotalPrice"){
