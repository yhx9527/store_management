    ...
    <el-card style="width: 50vw;margin:auto;margin-top: 30px;" shadow="hover">
      ...
      >
        ...
        <el-form-item label="用户权限" prop="userInfoRoles">
          <el-select
            v-model="userInfoForm.userInfoRoles"
            placeholder="请选择用户权限"
            style="float: left;"
          >
        ...
        <el-form-item style="text-align: left;">
          <el-button
            :type="ifadd ? 'primary' : 'warning'"
            @click="submitForm('userInfoForm')"
            style="margin-right: 10px;"
          >{{ifadd ? '立即添加' : '确认修改'}}</el-button>
          <el-button
            @click="resetForm('userInfoForm')"
            v-if='ifadd'
            type=""
          >重置</el-button>
          <el-popover v-else placement="top" width="160" v-model="visible2">
            <p>您确定要重置该用户的密码吗?</p>
            <div style="text-align: right; margin: 0">
              <el-button size="mini" type="text" @click="visible2 = false">取 消</el-button>
              <el-button type="primary" size="mini" @click="resetForm('userInfoForm')">确 定</el-button>
            </div>
  ...
  methods: {
    ...
    async resetForm(formName) {
      if (this.ifadd) {
        ...
        //console.log('uid', this.userInfoForm.userInfoId)
        //this.userInfoForm.userInfoPass = 123456;
        let data = await this.$apis.user_reset_pwd(this.userInfoForm.userInfoId)
        this.visible2 = false
      }
    }
  },
